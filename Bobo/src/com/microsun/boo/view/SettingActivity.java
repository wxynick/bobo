...
public class SettingActivity extends JPushBaseActivity implements OnClickListener {
   ...
   @Override
    ...
    private void initData(){
	  mSettings = getSharedPreferences(AppUtil.PREFS_NAME, MODE_PRIVATE);
	  String days = mSettings.getString(AppUtil.PREFS_DAYS, "");
	  ...
	  int startTimeStr = mSettings.getInt(AppUtil.PREFS_START_TIME, 0);
	  startTime.setCurrentHour(Integer.valueOf(startTimeStr));
	  int endTimeStr = mSettings.getInt(AppUtil.PREFS_END_TIME, 23);
	...
	private void setPushTime(){
		...
		mEditor.putString(AppUtil.PREFS_DAYS, daysSB.toString());
		mEditor.putInt(AppUtil.PREFS_START_TIME, startime);
		mEditor.putInt(AppUtil.PREFS_END_TIME, endtime);
		mEditor.commit();
